...
2. **Preprocesamiento NLP:** * Transforma emojis en texto.
   * Elimina signos de puntuación y *stopwords* (palabras vacías).
...
5. **Exportación:** Genera gráficos de optimización en la carpeta `/output` y un archivo `clustering_hibrido_tableau.csv` listo para visualizar.
...
### Bloque `clustering`
Controla el comportamiento matemático del LDA y el K-Means.
...
* **`lda_passes`**: Número de pasadas (épocas) que el modelo LDA da sobre el corpus completo durante su entrenamiento. Un número alto (ej. 50) mejora los resultados a costa de más tiempo de procesamiento.
